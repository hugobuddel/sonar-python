...
public class UselessStatementCheck extends PythonSubscriptionCheck {
  ...
  private static final List<Kind> regularKinds = Arrays.asList(Kind.NUMERIC_LITERAL, Kind.LIST_LITERAL, Kind.SET_LITERAL,
    Kind.QUALIFIED_EXPR, Kind.DICTIONARY_LITERAL, Kind.NONE, Kind.CONDITIONAL_EXPR, Kind.LAMBDA);
  ...
  private void checkStringLiteral(SubscriptionContext ctx) {
    ...
    if (!reportOnStrings || stringLiteral.stringElements().stream().anyMatch(StringElement::isTripleQuoted)) {
      return;
    }
    checkNode(ctx);
  }
